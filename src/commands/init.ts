...
# Auto-commit the renamed prompt files
# This prevents the user from needing a second commit
if [ -n "$(git status --porcelain .prompts/prompts/)" ]; then
  git add .prompts/prompts/ 2>/dev/null
  git commit --no-verify -m "chore: update prompt SHA references

Updated prompt file references from 'pending' to commit $COMMIT_SHA.

🤖 Auto-generated by gitify-prompt" 2>/dev/null || true

  if [ $? -eq 0 ]; then
    echo "✓ Auto-committed prompt SHA updates"
  fi
fi
